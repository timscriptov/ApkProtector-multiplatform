...
public class DexProtector {
    ...
    public void install(@NotNull Context context) throws Exception {
        String[] filelist = mContext.getAssets().list(Preferences.dexFolderName(context));
        for (int o = 0; o < filelist.length; o++) {
            if (filelist[o].endsWith("." + Preferences.getReplaceDexName(context))) {
                new DexDexryption(mContext, dexFiles).op(context, filelist[o]);
            }
        }
    ...
    public static void unzip(@NotNull Context context) throws Exception {
        File dexZipPath = new File(context.getFilesDir() + File.separator + "dex.zip");
        InputStream filelist = context.getAssets().open("dex.bin");
        DexDexryption.decDex(context, filelist, dexZipPath);
    }
}