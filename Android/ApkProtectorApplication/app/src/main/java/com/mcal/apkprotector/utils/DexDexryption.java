...
public class DexDexryption {
    @SuppressLint("StaticFieldLeak")
    public static Context mContext;
    private static List<File> mDexFiles = new ArrayList<>();

    public DexDexryption(Context context, List<File> dexFiles) {
        mContext = context;
        mDexFiles = dexFiles;
    ...
    public static void op(Context context, String str) throws Exception {
        File file = new File(context.getDir("apkprotector_dex", 0), str);
        if (!file.exists()) {
            file.mkdirs();
        }

        decDex(context, context.getAssets().open(Preferences.dexFolderName(context) + "/" + str), file);
        mDexFiles.add(file);
    }
    ...
    public static void decDex(Context context, InputStream is, @NotNull File file) {
        ...
        try {
            os = new InflaterOutputStream(new FileOutputStream(file));
            oss(context, is, os);
            a(os);
            a(is);
        } catch (Exception e) {
    ...
    private static void oss(Context context, @NotNull InputStream inputStream, OutputStream outputStream) throws Exception {
        char[] chars = Preferences.protectKey(context).toCharArray();
        int[] __ = new int[4];
        int i = 1;
        int i2 = i + 1;
