...
import mirror.android.util.TypedValue;
...
public class ARSCDecoder {
    ...
    public static ARSCData decode(InputStream arscStream, boolean findFlagsOffsets, boolean keepBroken,
        ...
        try {
            ...
            throw new AndrolibException("无法解码 arsc 文件", ex);
        }
    }
    ...
    private void readEntry() throws IOException, AndrolibException {
        ...
        if (size < 0) {
            throw new AndrolibException("入口大小小于 0 字节.");
        }
    ...
    private ResConfigFlags readConfigFlags() throws IOException, AndrolibException {
        ...
        if (size < 28) {
            throw new AndrolibException("配置文件大小小于 28");
        }
    ...
    private void checkChunkType(int expectedType) throws AndrolibException {
        if (mHeader.type != expectedType) {
            throw new AndrolibException(String.format("非法的块(chunk)类型: 理想=0x%08x, 实际=0x%08x",
    ...
    public static class ARSCData {
        ...
        public ResPackage getOnePackage() throws AndrolibException {
            if (mPackages.length <= 0) {
                throw new AndrolibException("Arsc 文件没有包含包名");
            } else if (mPackages.length != 1) {
                int id = findPackageWithMostResSpecs();
                LOGGER.info("Arsc 文件包含多个包名. 使用 "
                        + mPackages[id].getName() + " 作为默认.");
