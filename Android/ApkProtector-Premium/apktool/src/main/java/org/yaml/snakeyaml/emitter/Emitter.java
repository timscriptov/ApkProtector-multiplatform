...
public final class Emitter implements Emitable {
    ...
    private class ExpectStreamStart implements EmitterState {
        public void expect() throws IOException {
            if (event instanceof StreamStartEvent) {
                ...
                throw new EmitterException("需要 StreamStartEvent 事件, 但却得到了 " + event);
            }
        }
    }
    ...
    private class ExpectNothing implements EmitterState {
        public void expect() throws IOException {
            throw new EmitterException("什么事件都不需要, 但却得到了 " + event);
        }
    }
    ...
    private class ExpectDocumentStart implements EmitterState {
        ...
        public void expect() throws IOException {
            if (event instanceof DocumentStartEvent) {
                ...
                throw new EmitterException("需要 DocumentStartEvent 事件, 但却得到了 " + event);
            }
        }
    }
    ...
    private class ExpectDocumentEnd implements EmitterState {
        public void expect() throws IOException {
            if (event instanceof DocumentEndEvent) {
                ...
                throw new EmitterException("需要 DocumentEndEvent 事件, 但却得到了 " + event);
            }
        }
    }
    ...
    private void processTag() throws IOException {
        ...
        if (tag == null) {
            throw new EmitterException("标签没有被指定");
        }
        if (preparedTag == null) {
            preparedTag = prepareTag(tag);
        }
    ...
    private String prepareTagHandle(String handle) {
        if (handle.length() == 0) {
            throw new EmitterException("标签处理器不能为空");
        } else if (handle.charAt(0) != '!' || handle.charAt(handle.length() - 1) != '!') {
            throw new EmitterException("标签处理器必须以 '!' 开始和结束: " + handle);
        } else if (!"!".equals(handle) && !HANDLE_FORMAT.matcher(handle).matches()) {
            throw new EmitterException("标签处理器中含有非法字符: " + handle);
        }
        return handle;
    }
    ...
    private String prepareTagPrefix(String prefix) {
        if (prefix.length() == 0) {
            throw new EmitterException("标签前缀不可为空");
        }
    ...
    static String prepareAnchor(String anchor) {
        if (anchor.length() == 0) {
            throw new EmitterException("锚点不可为空");
        }
        if (!ANCHOR_FORMAT.matcher(anchor).matches()) {
            throw new EmitterException("锚点中含有非法字符: " + anchor);
        }
        return anchor;
    }
