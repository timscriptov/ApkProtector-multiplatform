...
// TypeSpecPtr is a managed pointer that knows how to delete itself.
    ...
    public:
        ...
        uint32_t
        FindEntryByName(const std::u16string &type_name, const std::u16string &entry_name) const;

        static const ResTable_entry *
        GetEntry(const ResTable_type *type_chunk, uint16_t entry_index);

        static uint32_t GetEntryOffset(const ResTable_type *type_chunk, uint16_t entry_index);

        static const ResTable_entry *
        GetEntryFromOffset(const ResTable_type *type_chunk, uint32_t offset);
        ...
        // the mipmap type.
        void
        CollectConfigurations(bool exclude_mipmap, std::set <ResTable_config> *out_configs) const;
...
// when loading, including offsets and lengths.
    ...
    private:
        ...
        bool
        LoadTable(const Chunk &chunk, const LoadedIdmap *loaded_idmap, bool load_as_shared_library);
