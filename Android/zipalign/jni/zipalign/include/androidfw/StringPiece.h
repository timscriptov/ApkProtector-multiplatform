...
// std::make_pair<StringPiece, T>(StringPiece(my_string), ...);
    ...
    public:
        ...
        BasicStringPiece(
                const std::basic_string <TChar> &str);  // NOLINT(google-explicit-constructor)
        BasicStringPiece(
                const TChar *str);                     // NOLINT(google-explicit-constructor)
...
//
    ...
    inline ::std::ostream &operator<<(::std::ostream &out, const BasicStringPiece<char16_t> &str) {
        ...
        utf16_to_utf8(str.data(), str.length(), &*result.begin(),
                      static_cast<size_t>(result_len) + 1);
        return out << result;
    }
    ...
    template<typename TChar>
    inline bool
    operator==(const ::std::basic_string <TChar> &lhs, const BasicStringPiece<TChar> &rhs) {
        return rhs == lhs;
    }
