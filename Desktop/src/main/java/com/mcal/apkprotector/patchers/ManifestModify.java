...
public class ManifestModify {
    ...
    private void addMetaData(Axml axml) {
        ...
        if (applicationNode != null) {
            ...
            dexFolderName.attr(NS_ANDROID, "name", android.R.attr.name, 3, "DexDir");
            dexFolderName.attr(NS_ANDROID, "value", android.R.attr.value, 3, CommonUtils.encryptStrings(Preferences.getDexDir(), 2));
            applicationNode.children.add(dexFolderName);

            Node replaceDexName = new Node();
            replaceDexName.name = "meta-data";
            replaceDexName.attr(NS_ANDROID, "name", android.R.attr.name, 3, "DexPrefix");
            replaceDexName.attr(NS_ANDROID, "value", android.R.attr.value, 3, CommonUtils.encryptStrings(Preferences.getDexPrefix(), 2));
