...
public class ResourceApkBuilder {
  ...
  @Contract(pure = true)
  private @NotNull String formatHashAlgorithName(@NotNull String hash) {
    return hash.replace("-", "");
  }
  ...
  private void signWithV2V3Sign(@NotNull File unSignedApk, @NotNull File signedApk, int minSDKVersion, InputParam.SignatureType signatureType) throws Exception {
  ...
  private void alignApk(@NotNull File before, File after) throws IOException, InterruptedException {
  ...
  private void addNonSignatureFiles(List<File> collectFiles, @NotNull File metaFolder) {
  ...
  private void addStoredFileIn7Zip(@NotNull ArrayList<String> storedFiles, File outSevenZipAPK)
      throws IOException, InterruptedException {
  ...
  private void generalRaw7zip(@NotNull File outSevenZipApk) throws IOException, InterruptedException {
