...
//            System.exit(1);
            throw new AndrolibException("SmaliDebugging 在版本 2.1.0 已经被移除了. 具体请查阅: https://github.com/iBotPeaches/Apktool/issues/1061");
        }
...
//            System.exit(1);
            throw new AndrolibException("输出目录 ("
                    + outDir.getAbsolutePath()
                    + ") "
                    + "已经存在. 如果需要覆盖它的内容, 请添加 -f 选项");
        } catch (InFileNotFoundException ex) {
//            System.err.println("Input file (" + apkName + ") " + "was not found or was not readable.");
//            System.exit(1);
            throw new AndrolibException("输入文件 (" + apkName + ") " + "未找到或不可读");
...
//            System.exit(1);
            throw new AndrolibException("无法找到 id 为: "
                    + String.valueOf(ex.getPkgId())
                    + " 的框架资源. 你需要安装一个正确的框架文件, "
                    + "了解更多请查看项目网站.");
        } catch (IOException ex) {
//            System.err.println("Could not modify file. Please ensure you have permission.");
//            System.exit(1);
            throw new AndrolibException("无法更改文件, 请确保有足够的权限");
        } catch (DirectoryException ex) {
//            System.err.println("Could not modify internal dex files. Please ensure you have permission.");
//            System.exit(1);
            throw new AndrolibException("无法更改内部dex文件, 请确保有足够的权限");
        }
