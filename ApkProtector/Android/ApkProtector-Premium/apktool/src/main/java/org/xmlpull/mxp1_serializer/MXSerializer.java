...
public class MXSerializer implements XmlSerializer {
	...
	public void setFeature(String name, boolean state)
		...
		if (name == null) {
			throw new IllegalArgumentException("功能名不可为 null");
		}
		if (FEATURE_NAMES_INTERNED.equals(name)) {
			...
			throw new IllegalStateException("不支持的功能 " + name);
		}
	}
	...
	public void setProperty(String name, Object value)
			throws IllegalArgumentException, IllegalStateException {
		if (name == null) {
			throw new IllegalArgumentException("属性名 null");
		}
		if (PROPERTY_SERIALIZER_INDENTATION.equals(name)) {
			...
			throw new IllegalStateException("不支持的属性 " + name);
		}
	...
	public void setOutput(OutputStream os, String encoding) throws IOException {
		if (os == null)
			throw new IllegalArgumentException("输出流不可为 null");
		reset();
		if (encoding != null) {
	...
	public void setPrefix(String prefix, String namespace) throws IOException {
		...
		if (!namesInterned) {
			...
			throw new IllegalArgumentException("前缀不可为 null"
					+ getLocation());
		}
		...
		for (int i = elNamespaceCount[depth]; i < namespaceEnd; i++) {
			if (prefix == namespacePrefix[i]) {
				throw new IllegalStateException("分离的前缀 "
		...
		if (!namesInterned) {
			...
			throw new IllegalArgumentException("命名空间不可为 null"
					+ getLocation());
		}
	...
	public XmlSerializer endTag(String namespace, String name)
		...
		if (namespace != elNamespace[depth]) {
			throw new IllegalArgumentException("需要命名空间 "
					+ printable(elNamespace[depth]) + " 而不是 "
					+ printable(namespace) + getLocation());
		}
		if (name == null) {
			throw new IllegalArgumentException("结束标签名不能为 null"
					+ getLocation());
		}
		if (checkNamesInterned && namesInterned) {
			checkInterning(name);
		...
		if ((!namesInterned && !name.equals(startTagName))
			...
			throw new IllegalArgumentException("需要节点名 "
					+ printable(elName[depth]) + " 而不是 " + printable(name)
					+ getLocation());
		}
		if (startTagIncomplete) {
			writeNamespaceDeclarations();
					...
					if (ch == 9 || ch == 10 || ch == 13) {
						...
						if (TRACE_ESCAPING)
							System.err.println(getClass().getName()
									...
									+ " " + printable(text));
						throw new IllegalStateException("字符 "
								+ Integer.toString(ch)
								+ " 在输出中是被禁止的" + getLocation()
